 ...
 <div class="content">
        ...
        <div class="header">
            ...
            <h1 class="page-title">订单</h1>
                ...
                <ul class="breadcrumb">
            <li><a href="/SSMPropertys/home.do?flag=home">首页</a> <span class="divider">/</span></li>
...
<div class="btn-toolbar">
	<a href="/SSMPropertys/order.do?flag=orderAdd" role="button" class="btn btn-primary" data-toggle="modal">
    ...
    <table class="table table-striped table-bordered bootstrap-datatable datatable">
      ...
      <c:forEach items="${orderlist}" var="tbl">
        <tr>
          ...
          <td>
           <a href="/SSMPropertys/order.do?flag=orderPre&oid=${tbl.id}" class="btn btn-primary"> 预 览</a>
...
<div class="pagination">
    <ul>
    	<c:if test="${pre=='yes'}">
        <li><a href="/SSMPropertys/order.do?flag=order&pageNow=${pNow-1}">前一页</a></li>
    	</c:if>
        <c:forEach var="i" begin="1" end="${pageCount}">
        <li>
        <a href="/SSMPropertys/order.do?flag=order&pageNow=${i }">${i }</a>
        </li>
        </c:forEach>
        <c:if test="${next=='yes'}">
        <li><a href="/SSMPropertys/order.do?flag=order&pageNow=${pNow+1}">后一页</a></li>
        </c:if>
    </ul>
</div>             
                  <footer>
...
$(document).ready(function(){
  ...
  */
	...
	var url="/SSMPropertys/OrderServlet?oid="+$(this).attr('id');
...
function del(obj){
	...
	var delurl="/SSMPropertys/order.do?flag=delOrder&delid="+delid+"&pageNow="+pNow;
	$("#myModal").modal("show").on("shown", function () {
		$("#delbtn2").attr('href',delurl);
	});
}
